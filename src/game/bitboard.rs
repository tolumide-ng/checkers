...
use super::mvs::action::Action;
use super::mvs::path::ActionList;
...
impl BitBoard {
    ...
    fn get(&self, hor_mask: u64, shift: u8, turn: Player) -> Vec<ActionList> {
        // vertical mask
        ...
        let mut mvs: Vec<ActionList> = Vec::with_capacity(pcs.count_ones() as usize);
        ...
        while pcs != 0 {
            ...
            if enemy_on_target && valid_capture {
                ...
                // result.iter().for_each(|mv| mv.prepend(action));

                (kings != 0).then(|| {
                    let more = board.moves(!turn);
                    result.reserve(more.len());
                    result.extend(more);
                    // more.iter().for_each(|mv| {});
                    // result = &result | &more;
                });

                jumps.reserve(result.len());
                jumps.extend(result);
                // let result: HashSet<ActionList> =
                //     result.into_iter().filter(|x| x.capture).collect();
            ...
            jumps.into_iter().for_each(|mut acl| {
                acl.prepend(action);
                mvs.push(acl);
            });

            mvs.push(ActionList::from(action));
        }

        mvs
    }

    pub(crate) fn moves(&self, play_as: Player) -> Vec<ActionList> {
